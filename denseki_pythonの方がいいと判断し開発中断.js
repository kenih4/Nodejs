...
var mscharge = new Sig('mscharge');
mscharge.sid = '535637';
mscharge.signame = 'xfel_mon_ct_ms/charge';
mscharge.srv = 'srweb-dmz-03.spring8.or.jp';

var routeinf = new Sig('routeinf');
routeinf.sid = '547927';
routeinf.signame = 'xfel_tmg_sw_master_ema_route_inf_shm/current';
routeinf.srv = 'srweb-dmz-03.spring8.or.jp';
...
/* download operation stat */
	...
	if(now.getHours() >= 0 && now.getHours() < 8) {
		stadate = dateformat(now.setDays(now.getDays()-1), 'yyyy/mm/dd+17:00:00');
		stodate = dateformat(now.setDays(now.getDays()-1), 'yyyy/mm/dd+1:00:00');
	}else if(now.getHours() >= 8 && now.getHours() < 16){
		stadate = dateformat(now, 'yyyy/mm/dd+1:00:00');
		stodate = dateformat(now, 'yyyy/mm/dd+9:00:00');
	}else if(now.getHours() >= 16){
		stadate = dateformat(now, 'yyyy/mm/dd+9:00:00');
		stodate = dateformat(now, 'yyyy/mm/dd+17:00:00');		
	}

	

//	var stodate = dateformat(now, 'yyyy/mm/dd+HH:MM:ss');
//	var stadate = dateformat(now.setSeconds(now.getSeconds()-1), 'yyyy/mm/dd+HH:MM:ss');
//	stodate = "2018/11/02+11:50:32";
//	stadate = "2018/11/02+11:50:31";
	console.log(stadate);
	console.log(stodate);

	routeinf.data = await download_accsyncdac(routeinf.srv, routeinf.sid, stadate,stodate, 1);
	console.log(routeinf.name + '\t' + routeinf.signame +  '\t' + routeinf.data);
	//var val = await max(routeinf.data);
	console.log('val\t' + val);

	process.exit(1);

	mscharge.data = await download_accsyncdac(mscharge.srv, mscharge.sid, stadate,stodate, 1);
	console.log(mscharge.name + '\t' + mscharge.signame +  '\t' + mscharge.data);
	var val = await max(mscharge.data);
	console.log('val\t' + val);
...
async function max(dat) {
	...
	*/
		...
		console.log( "	max:	" +	max(dat) ); 
		return max(dat);
}

//-------------------------------------------------------------------------------------
async function keisan(dat) {
	/*
		dat.forEach(function(v, i, a){
		  console.log(v);
		});
	*/
		...
		console.log( "average:	" +	average(dat) +	"	min:	" +	min(dat)	+	"	max:	" +	max(dat) ); 
